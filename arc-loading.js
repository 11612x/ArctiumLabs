...
(function (global) {
  ...
  function injectStyles() {
    ...
    style.textContent =
      ...
      '.arc-loading-overlay{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;' +
      'backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);background:rgba(13,15,14,0.2);pointer-events:all}' +
      ...
      'transform-origin:center center;animation:arc-loading-spin 1.5s steps(12) infinite}';
  ...
  async function withArcLoadingViaParent(task) {
    notifyParent('start');
    await arcLoadingPaintBuffer();
    const started = performance.now();
    try {
  ...
  async function withArcLoading(task) {
    if (isEmbedded()) {
      try {
        if (typeof global.parent.withArcLoading === 'function') {
          return await global.parent.withArcLoading(task);
        }
      } catch (_) {}
      return withArcLoadingViaParent(task);
    }
  ...
  if (typeof global.withArcLoading !== 'function') {
